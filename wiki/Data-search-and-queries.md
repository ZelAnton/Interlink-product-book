...
В принятом плане [общая адресация](Elements-and-concepts) позволит находить не только самостоятельные карточки, но и поддержанные определения и части, например нужный переход маршрута. Результат показывает вид цели и разрешённые действия; найденный переход нельзя редактировать в обход его владельца.

[Словарь понятий](Domain-concepts) добавляет проверенные синонимы и сопоставления. Это помощь поиску, а не доказательство пригодности найденной детали или права на её использование. Закрытые заголовки и смысловые связи не должны раскрываться через общий индекс. Ссылка на сегодняшний предмет не выдаётся за его точное историческое содержание.

...
flowchart LR
    ...
    E --> F["Снимок передачи"]
    F --> G["ПВ, ERP и MES"]
    G --> H["Экземпляры, партии и факты"]
    H --> I["Перепланирование или сервис"]
    I -.-> B
```
...
Создаётся полный производственный снимок, затем ПВ и данные для ERP. При запуске появляются пять серийных экземпляров экскаваторов. Ковши могут получить отдельные номера, а ЗИП — номера комплектов или партий по правилам предприятия.

Через месяц поставщик меняет арктический рукав. Новая версия заказа применяется только к двум ещё не собранным северным машинам. Первый снимок и фактический состав уже изготовленной машины сохраняют прежний рукав.
...
После выпуска документа создаются подписанная ПВ, производственный снимок и ограниченный пакет для кооператора. Кооператор получает только чертежи рамы и требования к покрытию, но внутренний полный снимок остаётся целостным.
...
Заказ содержит одну позицию с количеством десять и условием двух этапов поставки. Формируется точный состав. Для первой очереди предприятие создаёт передачу на пять станций, ПВ и задания MES.
...
Технолог и служба качества согласуют изменения. Новая ПВ и новый снимок относятся ко второй очереди. В истории видно, что первая и вторая половины заказа изготовлены по разным производственным решениям.

### Передача в ERP и MES

ERP получает новую потребность только для оставшихся пяти станций и учитывает остатки старых уплотнений отдельно. MES получает точные новые документы. Повтор сообщения после сетевого сбоя не создаёт второй производственный заказ.

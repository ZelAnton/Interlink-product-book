...
flowchart TD
    ...
    E -->|Да| F["Использовать строго указанную версию; не переходить к более слабым правилам"]
    E -->|Нет| S{"Строгий режим требует фиксации каждой позиции?"}
    S -->|Да| T["Нет результата: позиция не зафиксирована"]
    S -->|Нет| G{"Есть рабочая версия в пакете изменения?"}
    G -->|Да| H["Использовать рабочую версию только для участников изменения"]
    G -->|Нет| P{"Есть временное предпочтение пакета?"}
    P -->|Да| Q["Использовать предпочтение только в этом пакете"]
    P -->|Нет| I{"Есть подходящая применяемость?"}
    Q --> M
...
Готовый снимок вообще не входит в живой порядок: он читается напрямую, до проверки присутствия и правил. Повреждённый или неподходящий снимок даёт отказ и не заменяется живым расчётом. Для живого подбора порядок можно записать одной строкой: **жёсткая фиксация → ранняя остановка строгого режима без фиксации → рабочая версия пакета → временное предпочтение пакета → применяемость → правило → заранее разрешённый резервный алгоритм или отсутствие версии**.
...
Общие правила подбора выпускаются как часть модели предприятия, а назначение профиля конкретной операции — через версионируемые настройки принимающего продукта. Предметный пакет изделия проводит применяемость, фиксации и другие данные конкретного объекта, но не является способом выпуска общего алгоритма.
...
Головное изделие, серия, дата, комплектация, правило и пакет изменения передаются как явный контекст каждой операции подбора. Они не должны незаметно меняться из-за переключения другого окна. Новый контекст не нужен лишь для чтения уже указанной точной версии, истории всех версий, неверсионируемого объекта или готового снимка, потому что там система ничего не подбирает.

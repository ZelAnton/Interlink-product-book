...
Количество «10 шт.» не означает, что при создании заказа надо сразу породить десять объектов-экземпляров. Публикация снимка также не создаёт партии и экземпляры автоматически. Они появляются на стадии исполнения — до запуска или уже во время изготовления и регистрации фактов, когда производство действительно начинает различать единицы. Точный момент задаёт процесс предприятия: это может быть подготовка задания, маркировка, начало сборки, испытание или приёмка.

```mermaid
flowchart LR
    A["Заказ: 10 редукторов"] --> B["Опубликованный точный состав<br/>как заказано"]
    B --> C["Исполнение в принимающем продукте"]
    C --> D["Создание партии и экземпляров<br/>в момент, установленный предприятием"]
    D --> E["Запись операций, материалов<br/>и фактического состава"]
    E --> F["Приёмка и закрытие исполнения"]
    D --> G["Экземпляр № 001"]
    D --> H["Экземпляр № 002"]
...
Interlink должен поддержать выбранную политику и связь экземпляра с исходной передачей, но сам снимок не является командой «создать десять серийных объектов». Экземпляры и партии создаёт производственный контур перед началом исполнения или во время регистрации факта — в момент, установленный предприятием. После выполнения операций он записывает фактический состав и закрывает либо отклоняет исполнение.

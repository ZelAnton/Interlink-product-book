...
Этот комплект предназначен прежде всего для продуктовых специалистов IPS, конструкторов, технологов, специалистов по управлению конфигурацией и руководителей внедрения систем управления жизненным циклом изделия (PLM). Для чтения не требуется знание программирования, устройства базы данных или внутренних средств разработки Interlink.
...
| Условия подбора | Явный контекст подбора | Все исходные данные, с которыми формируется состав. |
...
| Зафиксированный снимок состава | Долговечный снимок | Неизменяемое свидетельство полного точного состава, опубликованного для выпуска, заказа или обмена. |
...
flowchart TD
    ...
    C -->|Нет| S{"Строгий режим требует фиксации?"}
    S -->|Да| T["Версия не найдена: позиция не зафиксирована"]
    S -->|Нет| E{"Есть рабочая версия в пакете изменения?"}
    E -->|Да| F["Показать рабочую версию участникам изменения"]
    E -->|Нет| P{"Есть временное предпочтение пакета?"}
    P -->|Да| Q["Показать предпочтённую версию для проработки"]
    P -->|Нет| G{"Есть версия, применимая для изделия, серии или даты?"}
...
Отдельно от этого, после полного формирования состава из уже зафиксированных версий система может опубликовать неизменяемый снимок. Рабочая версия в снимок не попадает: сначала проводится её пакет, затем состав проверяется заново. При чтении повреждённого или неподходящего снимка система отказывает, а не подменяет историю живым пересчётом.

Общие правила выбора выпускаются через модель предприятия, а назначение профиля операции — через версионируемые настройки принимающего продукта. Предметный пакет отдельного изделия для выпуска общего правила не используется.
...
- [[Как пользователи работают с подбором версий|Selection-user-work]] — роли, происхождение исходных условий, три режима работы, карточка результата, исправление ошибок и публикация.
- [[Как создаются и выпускаются правила подбора|Selection-rule-lifecycle]] — работа методиста, применяемость, фиксации, аналоги, конфигуратор, испытания и выпуск правил.
...
- строгий режим без точной фиксации немедленно возвращает отсутствие результата и не переходит к более слабым правилам;

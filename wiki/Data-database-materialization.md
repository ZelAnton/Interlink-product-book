...
После реализации генератора физической схемы Interlink будет создавать не универсальную таблицу «все объекты», а физическую схему, соответствующую конкретным предметным типам. Сейчас эта страница описывает целевой физический контракт: реальные таблицы, DDL, ограничения и индексы ещё не формируются автоматически.

- неверсионируемый тип будет получать собственную таблицу;
- версионируемый тип — таблицу устойчивых объектов и таблицу их версий;
...
- кортеж — таблицу с уникальным сочетанием слотов;
- часто используемый атрибут будет становиться типизированным столбцом;
...
Физический контракт PostgreSQL подробно зафиксирован, но генератор физической схемы, планировщик миграций и промышленное применение схемы ещё не реализованы. Поэтому преимущества являются обоснованной архитектурной гипотезой до завершения интеграционных и нагрузочных испытаний.

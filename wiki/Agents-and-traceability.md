...
[Адресуемые элементы](Elements-and-concepts) дают общий способ указать определение, инструкцию, конкретный шаг или результат. Но процедура, агент как исполнитель, его запуск, отдельная попытка и делегирование остаются разными предметами. [Применение процедуры](Instructions-and-execution) ещё не доказывает её выполнение.

Для разбора решения сохраняются фактические входы и показанные агенту сведения, нужные исторические состояния и результаты действий. Общая ссылка не замыкает всю историю автоматически. [Смысловой поиск](Domain-concepts) помогает найти кандидатов, но не выдаёт полномочия и не подтверждает совместимость. Ручное и автономное выполнение сохраняют свои основания доступа и ответственности.

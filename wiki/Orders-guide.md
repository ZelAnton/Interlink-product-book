...
Для чтения заказного контура полезна новая глава [об инструкциях, применениях и выполнении](Instructions-and-execution). Два перехода в маршруте могут использовать одну инструкцию, но сохраняют собственные параметры; фактическая работа по конкретному экземпляру не сливается с её планом. Изменение будущих шагов начатого задания оформляется отдельно и не переписывает завершённое.

[Адрес элемента](Elements-and-concepts) помогает сохранить связи замечаний, переходов и свидетельств. Он не заменяет точную выдачу заказа и не добавляет прав на изменение чужого содержания. Эти различия входят в принятый план, их сквозная реализация ещё требует подтверждения.

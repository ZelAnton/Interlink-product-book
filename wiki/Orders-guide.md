...
После согласования выпускается версия производственного заказа (ПЗ). Для конкретной официальной передачи отдельным решением публикуется **неизменяемый снимок точного состава**; затем принимающий продукт доставляет его во внешнюю систему и отдельно фиксирует результат приёма. Выпуск версии ПЗ, публикация снимка, доставка и приём — четыре разных события. Одна версия заказа может иметь несколько снимков, а новый снимок не переписывает предыдущий.

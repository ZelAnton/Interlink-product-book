...
## Словарь и классификация не меняют поставленные правила

[Предметные понятия](Domain-concepts) развивают управляемую классификацию, а не создают второй несвязанный каталог. Одно понятие может занимать несколько мест в разных схемах; навигационная папка не обязана обозначать самостоятельный смысл.

Поставляемые с моделью структурные привязки входят в её управляемый выпуск. Прикладное назначение понятия или замечание к защищённому определению хранится отдельно и не меняет обязательные свойства типа. Назначение метки не запускает действие классификатора: такое действие остаётся явной командой с правами и проверками. Для прежнего решения сохраняются использованные редакции понятий и существенных настроек.

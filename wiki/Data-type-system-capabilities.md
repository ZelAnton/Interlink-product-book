...
Принятый договор [элементов](Elements-and-concepts) объединяет проверяемую ссылку на тип, предметные данные и объявленную часть. Он не объединяет их команды изменения. Замечание к обязательному полю типа может храниться отдельно; исправление самого определения проходит управляемый выпуск модели.

Тип «Инструкция обработки» задаёт форму данных. Инструкция для посадочной шейки — данные этой формы. Два применения в маршруте вала имеют свои параметры и устойчивые адреса, а не два новых типа. Классификация инструкции понятием «Чистовое точение» объясняет смысл, но не меняет её структуру и не запускает обработку. Подробнее — [процессы](Instructions-and-execution) и [словарь понятий](Domain-concepts).

...
flowchart TD
    ...
    G --> B
    F -->|"результат однозначен"| H["Выпуск заказа и точный снимок"]
```
...
Для каждой включённой позиции выбирается точная допустимая версия с учётом степени готовности, применяемости, рабочей области и явной фиксации. Подробные сценарии приведены в отдельном [пакете подбора версий](Version-selection-guide).
...
Порядок формирования и его продуктовые гарантии описывают целевое состояние. В базовом Interlink уже определены контекст, применимость, предпочтения версий и адрес позиции. Многоуровневое раскрытие, конфигуратор, варианты, технология и заказная публикация относятся к последующим этапам.

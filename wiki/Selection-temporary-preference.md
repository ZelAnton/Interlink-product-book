...
Временное предпочтение существует только внутри конкретного пакета изменения. Если для объекта в этом же пакете есть собственная рабочая версия, она имеет больший приоритет: предпочтение не должно заслонять фактически изменяемое будущее содержание.
...
stateDiagram-v2
    ...
    ВременныйВыбор --> ЖесткаяФиксация: отдельно создана постоянная фиксация
    ВременныйВыбор --> РешениеЗаказа: согласовано только для одной передачи
    ВременныйВыбор --> Ошибка: версия недопустима или решение не принято
    Удалено --> [*]
    ЖесткаяФиксация --> Удалено: временное предпочтение удалено
    РешениеЗаказа --> Удалено: решение сохранено только в новом снимке
    Ошибка --> [*]
```

Само временное предпочтение всегда удаляется до проведения пакета. Возможны три предметных исхода:

1. удалить временное предпочтение и вернуться к применяемости или общему правилу;
2. отдельно создать жёсткую фиксацию версии в новой версии родительского состава, после чего удалить предпочтение;
3. если уже зафиксированная версия нужна только для одной передачи заказа, принять отдельное согласованное решение и сохранить его только в новом снимке этой передачи, затем удалить предпочтение.

Оставить временное предпочтение «как-нибудь после выпуска» нельзя. Жёсткая фиксация действует столько же, сколько сама новая версия родительского состава. Разовое решение заказа не становится общей фиксацией или резервным правилом, а снимок может ссылаться только на уже зафиксированную версию — не на рабочую версию пакета.
...
- версия 03 обязательна только в этой позиции — отдельно создаётся жёсткая фиксация в новой версии родительского состава, а временное предпочтение удаляется.

Если версия 03 уже зафиксирована и нужна только для одной согласованной передачи заказа, принимающий продукт может оформить отдельное решение заказа и новый снимок. Предпочтение при этом удаляется и не влияет на следующий заказ.
...
- `Перед проведением удалите временное предпочтение; при необходимости отдельно оформите фиксацию версии 03`;
...
7. Разовое решение заказа сохраняется только в новом снимке конкретной передачи и не оставляет временное предпочтение активным.

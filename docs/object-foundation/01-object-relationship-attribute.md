...
## Адресуемые части дополняют объекты, связи и свойства

Уточнение от 7 сентября не сводится к переименованию объекта в сущность. [Общий элемент](04-elements-and-concepts.md) позволяет адресовать также определение модели или объявленную часть владельца. Например, переход внутри маршрута может сохранять устойчивый локальный адрес и историю вместе с маршрутом, не получая отдельную инженерную версию.

Две обработки по одной [инструкции](05-instructions-and-execution.md) остаются разными применениями; физический вал, выполнение и измерение также различаются. [Предметное понятие](06-domain-concepts.md) описывает смысл и не задаёт способ хранения или изменения. Общие гарантии ниже сохраняются, но не превращаются в одинаковые команды для всех видов данных.

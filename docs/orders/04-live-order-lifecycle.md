...
Пока специалисты готовят следующую версию, заказ может учитывать актуальные разрешённые версии изделий и документов. После выпуска версии заказа отдельной операцией публикуется неизменяемый снимок конкретной передачи, и уже этот снимок отправляется получателю. Поэтому развитие заказа и доказательство прошлой передачи не конфликтуют, а внешняя доставка не подменяет публикацию.
...
stateDiagram-v2
    ...
    Выпущен --> Опубликован: создан точный снимок передачи
    Опубликован --> Передан: снимок отправлен получателю
    Передан --> Исполнение: получатель принял передачу
    Исполнение --> Закрыт: работа завершена
    Исполнение --> НоваяВерсия: требуется перепланирование
    НоваяВерсия --> Анализ
    Подготовка --> Отменён: работа прекращена до выпуска
    Анализ --> Отменён: работа прекращена до выпуска
    Закрыт --> [*]
    Отменён --> [*]
```
...
## Этап 5. Публикация, передача и дальнейшее развитие

Для производства, 1С, архива или другого существенного получателя формируется свой предметный корень передачи и отдельным действием публикуется снимок. Только опубликованный снимок можно считать основанием внешнего сообщения. Состояния «опубликован», «ожидает отправки», «принят», «частично принят», «отклонён» и «исход неизвестен» не должны сливаться в одну отметку «передан».

После этого заказ может получить новую версию: изменить количество, комплектацию, допустимую замену или состав поставки. Новый результат публикуется и передаётся новым снимком, а прежний остаётся доступен без пересчёта. Если содержание заказа не менялось, другая самостоятельная передача может получить новый снимок той же версии заказа.
...
Специалист создаёт новую версию заказа, меняет количество и повторно проверяет потребности. Первый снимок продолжает относиться к прежней передаче десяти редукторов. Для следующей передачи предприятие выбирает и заранее закрепляет один из двух предметных вариантов: создать новый корень полной передачи на двенадцать изделий либо создать отдельное добавочное задание на две единицы и явно связать его с приростом заказа. Снимок фиксирует полный состав выбранного корня; он сам не вычитает две единицы из строки заказа и не ведёт нераспределённый остаток.
...
Если производству ещё ничего не передавали, специалист может принять новую версию в текущей подготовке. Если первая очередь уже передана, старый снимок не меняется. Для остатка сначала выпускается новая версия заказа либо самостоятельное задание следующей очереди с явным распределением количества, и только этот предметный корень становится основанием нового снимка.
...
Основания: [IPS-A13], [IPS-M03], [IL-ADR ADR-052], [IL-HOST §8]. Расшифровка — в [реестре источников](../knowledge/15-source-register.md).

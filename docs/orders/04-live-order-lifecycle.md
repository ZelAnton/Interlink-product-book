...
[Контекст подбора](../version-selection/21-selection-contexts.md) отдельно задаёт, какие версии предпочитать при проверке будущего результата. Он может использоваться несколькими рабочими областями и переживать отдельный пакет. Постоянное мягкое предпочтение позиции также не исчезает только потому, что пакет завершён.
...
## Почему новая публикация не меняет прошлую передачу

```mermaid
sequenceDiagram
    participant П as Планировщик
    participant З as Живой заказ
    participant С as Исторический снимок
    П->>З: Сохранить новый маршрут второй очереди
    З-->>П: Рабочая правка, прежнее официальное основание сохранено
    П->>З: Проверить и опубликовать новое содержание
    З-->>П: Живой расчёт может использовать новый маршрут по своим правилам
    П->>С: Открыть первую передачу
    С-->>П: Показать прежний точный маршрут и состав
```

Рабочая правка сначала доступна только явно подключившим её участникам. После публикации новый результат используют живые расчёты, чьи условия выбирают его. Историческая передача читает прежнее содержание без повторного подбора. Если для второй очереди требуется отправка, она оформляется отдельно; чтение нового маршрута не выдаёт новое задание само по себе.
...
Основания: [IPS-A13], [IPS-M03], [IL-ORDERS], [IL-KERNEL], [IL-PLAN-V2]. Расшифровка и статус материалов — в [реестре источников](../knowledge/15-source-register.md).

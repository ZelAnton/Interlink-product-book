...
## Выпуск состоялся, а ответ на отправку потерян

Этот путь особенно важен при автоматической передаче заданий. Он показывает, что надёжная локальная фиксация и успешный внешний приём — разные результаты. В примере договор получателя позволяет достоверно сверить прежнее задание по его устойчивому номеру.

```mermaid
sequenceDiagram
    actor U as Планировщик
    participant P as Принимающий продукт
    participant E as Внешняя производственная система
    U->>P: Выпустить согласованный комплект
    P->>P: Вместе сохранить выпуск, журнал и задание доставки
    P-->>U: Локальный выпуск подтверждён
    P->>E: Передать прежнее определённое задание
    Note over P,E: Ответ на отправку потерян
    P-->>U: Исход приёма неизвестен, количество не освобождено
    P->>E: Сверить исход той же передачи
    E-->>P: Сообщить достоверный результат либо неизвестность
    P-->>U: Показать результат сверки и разрешённое действие
```

Неизвестность после сверки остаётся неизвестностью: она не разрешает новое задание на то же количество. При подтверждённом приёме система продолжает прежнюю историю без повторного назначения. При достоверном отсутствии эффекта или наличии согласованной защиты от повторного выполнения возможен безопасный повтор прежнего намерения; новый номер не используется для обхода неопределённости.
...
Карточка передачи показывает эти линии раздельно. Для выпуска видно, подготовлен ли комплект или уже опубликован; для доставки — ожидает ли он отправки, отправляется, доставлен либо требует сверки; для приёма — принято ли задание полностью, частично, отклонено или его исход неизвестен. Эти отметки не заменяют друг друга: опубликованный комплект может ещё не быть доставлен, а доставленный — не быть принят. Для частичного приёма карточка показывает конкретные области и допускает только предусмотренное договором действие: дослать ту же операцию, оформить исправление, разрешённую отмену либо передать случай на сверку.
...
Сам файл обмена остаётся формой передачи. [Справочные таблицы](../tabular-data/01-reference-tables.md) и [координатные карты](../tabular-data/03-coordinate-maps.md) позволяют искать исходные данные внутри системы, но чтение новой строки не запускает автоматически обновление карточек или внешних заданий. Такая синхронизация оформляется отдельной проверяемой операцией.
...
Текущий целевой договор и планы Interlink: [IL-HOST §8], [IL-ORDERS], [IL-KERNEL], [IL-TD], [IL-SC], [IL-PLAN-V2]. Требования и практика IPS: [IPS-A13], [IPS-A23], [IPS-M03]. Историческое [IL-ADR ADR-052] объясняет происхождение решений; при уточнении поведения приоритет имеют нынешние договоры Interlink.

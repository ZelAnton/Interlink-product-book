...
```mermaid
flowchart LR
    A["Заказ: 10 редукторов"] --> B["Точный состав<br/>как заказано"]
    B --> C["Производственная партия<br/>10 шт."]
    C --> D["Экземпляр № 001"]
    C --> E["Экземпляр № 002"]
...
Единое правило для всех предприятий невозможно. В разных процессах серийный номер или полноценная запись физического экземпляра могут появляться в разные моменты:
...
Interlink должен поддержать выбранную политику, но не создавать экземпляры автоматически на стадии коммерческого заказа без предметного основания. Обычный ранний путь — создать или связать партию и экземпляры после принятой производственной передачи и до записи относящихся к ним фактов исполнения. Если предприятие регистрирует полноценный экземпляр только при испытании, приёмке или отгрузке, предшествующие производственные факты должны иметь устойчивое временное основание и затем однозначно связаться с созданным экземпляром. Схема не должна позволять «потерять» исполнение до позднего присвоения номера.
...
Основания: [IPS-A13], [IPS-M03], [IL-ADR ADR-052]. Расшифровка — в [реестре источников](../knowledge/15-source-register.md).

...
## Экземпляр не равен выполнению операции

Физический вал или насос сохраняет собственную идентичность, а обработки, испытания и ремонты относятся к отдельным прохождениям работы. [Применение инструкции](../object-foundation/05-instructions-and-execution.md) указывает плановое место, выполнение — что произошло с данным экземпляром или партией, результат — что было зафиксировано и принято.

При доработке корпуса создаётся новое выполнение с основанием и результатами; исходное измерение сохраняется. При повторной передаче того же сообщения после сбоя новое выполнение не возникает. Точный разбор должен открыть прежние маршрут, инструкцию и свидетельства, а не объяснять старую работу сегодняшними требованиями.

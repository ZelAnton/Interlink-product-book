...
| Производное представление принимающего продукта | Отчёт или форма обмена по точному основанию; не дополнительный вид снимка | Может формироваться заново; прежнее подписанное свидетельство удерживается отдельно по своим обязательствам |
...
Частичная поставка определяется не произвольным числом, переданным при создании снимка, а предметной областью. Для строки ПЗ на десять насосных станций принимающий продукт сначала создаёт самостоятельную область первой очереди и назначает ей пять единиц либо публикует новое состояние ПЗ с явно разделёнными очередями. Снимок раскрывает уже определённый корень и никогда сам не делит десять на пять и пять. Оставшееся количество продолжает учитываться вместе с прежними передачами.
...
В этом сценарии первая выдача была заранее оформлена именно на пять редукторов: определён корень первой очереди, ему назначено количество и создан снимок. Остальные пять оставались нераспределёнными. Если предприятие ранее выдало всё количество десять, сначала требуется согласованно изменить соответствующее прежнее обязательство; простое создание ещё одной выдачи на пять было бы двойным назначением.
...
### Почему архив и отчёт различаются

```mermaid
sequenceDiagram
    actor A as Архивист
    participant P as Принимающий продукт
    participant D as Точный комплект оснований
    A->>P: Подготовить архив конкретной выдачи
    P->>D: Прочитать прежний снимок и необходимые основания
    D-->>P: Точные состояния, файлы, подписи и правила чтения
    P->>P: Проверить полноту, доступ и обязательства хранения
    alt Все обязательные материалы доступны
        P-->>A: Самостоятельный архивный комплект с источниками
    else Обязательный материал утрачен или недоступен
        P-->>A: Неполный результат, не выдавать за полный архив
    end
```

Схема показывает подготовку самостоятельного архивного свидетельства. Перед помещением архива на хранение принимающий продукт проверяет наличие каждого обязательного файла, его содержание, подпись и допустимость доступа. Один адрес файла не гарантирует доступности его прежнего содержимого через двадцать лет. Новая форма отчёта может быть построена по тому же основанию, но она не переписывает снимок или подписанный архив. Если сама архивная выдача является новой деловой публикацией по правилам предприятия, для неё предварительно создаётся собственный снимок.
...
11. Нельзя выдать пять единиц из строки на десять без явной области и проверяемого распределения; остаток не исчезает.
12. Заменяемый отчёт не выдаётся за долговечный снимок или прежний подписанный файл; автономный архив имеет собственную проверку полноты и обязательства удержания.
...
Материалы IPS: [IPS-A13], [IPS-M03]. Действующие границы публикации, точности и доставки: [IL-ORDERS], [IL-KERNEL], [IL-SC], [IL-TD], [IL-DATA-MODEL], [IL-PLAN-V2]. История решения: [IL-PLM §5], [IL-HOST §8], [IL-ADR ADR-052]; прежние формулировки не заменяют актуальный договор удержания и определения самостоятельной передачи. Расшифровка и границы достоверности — в [реестре источников](../knowledge/15-source-register.md). Продуктовая редакция сверена с нормативной документацией Interlink на 6 сентября 2026 года; наличие принятых контрактов не означает готовность всего пользовательского процесса.

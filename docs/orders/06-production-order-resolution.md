...
flowchart TD
    ...
    C --> D["Подбор точных версий"]
    D --> E["Маршруты и технологические данные"]
    E --> F["Проверка полноты и объяснение решений"]
    F -->|"есть ошибка"| G["Исправление данных или ручное решение"]
    G --> B
    F -->|"результат однозначен"| H["Выпустить официальную версию заказа"]
    H --> I["Отдельно опубликовать точный снимок передачи"]
    I --> J["Передать снимок получателю"]
```
...
- рабочие предпочтения из текущего пакета изменения;
...
Если в позиции допускаются основной вариант, аналог или замена, сначала определяется допустимое изделие. Выбор версии выполняется уже для выбранного изделия. Это не позволяет случайно подменить выбор версии выбором другой номенклатуры.

### 4. Подбор версии

Для каждой включённой позиции выбирается точная допустимая версия с учётом состояния зрелости, применимости, рабочей области и явной фиксации. Подробные сценарии приведены в отдельном [пакете подбора версий](../version-selection/README.md).

### 5. Наложение технологии

После получения точного инженерного состава добавляются маршруты, операции, нормы материалов и труда, а также производственные документы. Технология может зависеть от версии детали, места её применения, предприятия и заказа.
...
- **предупреждение** — результат допустим, но требует внимания;
...
| Работать с аналогами и заменами | Отдельный предметный выбор изделия до выбора его версии |
...
| Контролируемо изменить результат | Новая версия заказа, повторный расчёт и новый снимок передачи |
...
Для электродвигателя найдены две версии: одна выпущена, другая ещё согласуется в рабочем пакете. При обычной публикации выбирается выпущенная. В совместном изменении конструктор может проверить будущую версию, но передача возможна только после разрешённого выпуска.

Технологический маршрут выбирается для северной площадки. Если маршрут покраски не определён, система показывает блокирующую ошибку именно на кожухе, а не выдаёт общий текст «заказ не сформирован».
...
Специалист явно выбирает аналог и видит, что для него требуется другой переходной фланец. Система пересчитывает только затронутые ветви, подбирает версии чертежей фланца и проверяет маршрут его изготовления. Отдельно поставляемые кулачки остаются самостоятельной строкой заказа и не превращаются в установленную часть станка.
...
Порядок формирования и его продуктовые гарантии описывают целевое состояние. В Interlink уже определены носители контекста, применяемости, предпочтений версий и адреса позиции, но это ещё не означает исполняемый сквозной подбор. Многоуровневое раскрытие, выполнение применяемости, конфигуратор, варианты, технология и заказная публикация относятся к последующим этапам.

## Связанные документы

- [Формирование состава: IPS → Interlink](../knowledge/05-composition-resolution.md)
- [Пакет сценариев подбора версий](../version-selection/README.md)
- [Маршруты, нормы и обеспечение](07-routes-norms-and-sourcing.md)

Основания: [IPS-A05], [IPS-A13], [IPS-A23], [IPS-M03], [IL-PLM §4–5], [IL-ADR ADR-052]. Расшифровка — в [реестре источников](../knowledge/15-source-register.md).

...
В частном случае, когда разрешение аналога принадлежит исходной версии, порядок работы выглядит так:

```mermaid
sequenceDiagram
    actor P as Специалист
    participant S as Формирование заказа
    participant D as Разрешения и инженерные данные
    P->>S: Рассмотреть замену двигателя в указанном месте
    S->>D: Установить исходную версию и прочитать её допуск
    D-->>S: Разрешённые альтернативы и условия
    S-->>P: Показать полный вариант с переходным фланцем
    P->>S: Выбрать вариант для этого заказа
    S->>D: Подобрать содержание заменителя и фланца
    D-->>S: Конкретные состояния и точные основания
    S-->>P: Итог совместимости, либо причина отказа
```

Сначала выясняется, чьё разрешение применяется, затем выбирается замена, и только после определения её содержания можно завершить проверку комплекта. Ни отказ, ни отсутствие допуска не разрешают системе самостоятельно перейти к разрешению другой исходной версии.

Для каждой включённой позиции выбирается инженерная версия, затем её содержание по заданному режиму: рабочие данные для проработки либо конкретное опубликованное состояние для выдачи. Жёсткое закрепление версии не тождественно фиксации её содержания на определённую дату. Подробные сценарии приведены в отдельном [пакете подбора версий](../version-selection/README.md).
...
Карта кандидатов сама не устанавливает уплотнение и не подтверждает совместимость всего насоса. Выбор, проверка и применение остаются явными действиями. См. [справочные таблицы](../tabular-data/01-reference-tables.md), [координатные карты](../tabular-data/03-coordinate-maps.md) и [матрицы совместимости](../substitution-and-compatibility/02-compatibility-matrices.md).
...
- [Формирование состава: предметные основания](../knowledge/05-composition-resolution.md)
...
Материалы IPS: [IPS-A05], [IPS-A13], [IPS-A23], [IPS-M03]. Действующая целевая модель: [IL-ORDERS], [IL-KERNEL], [IL-SC], [IL-TD], [IL-DATA-MODEL], [IL-PLAN-V2]. История проектирования: [IL-PLM §4–5], [IL-ADR ADR-052]. Подтверждённое назначение аналога для версии не означает, что из источника известен весь внутренний порядок конкретной установки IPS. Расшифровка и границы достоверности — в [реестре источников](../knowledge/15-source-register.md). Продуктовая редакция сверена с нормативной документацией Interlink на 6 сентября 2026 года; наличие принятых контрактов не означает готовность всего пользовательского процесса.

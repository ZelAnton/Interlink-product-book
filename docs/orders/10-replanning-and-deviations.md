...
Предприятие выбирает способ передачи: отдельный корень полного нового результата на двенадцать изделий либо самостоятельное добавочное задание на две единицы, связанное с приростом заказа. В обоих случаях прежний снимок на десять сохраняется. Новый снимок строится из выбранного корня передачи и не делит количество исходной строки сам. Правило полной или добавочной передачи и учёта остатка должно быть согласовано заранее.
...
Система показывает замену номенклатуры, затронутую дочернюю деталь и изменение комплекта документов. Технолог подтверждает новый маршрут сборки. Принимающее приложение создаёт самостоятельный корень следующей передачи, относит к нему оставшиеся пять единиц и проверяет, что распределение не пересекается с первой очередью. Interlink публикует полный снимок этого корня; сам снимок не «обрезает» исходный заказ. Фактические экземпляры первых пяти сохраняют исходную родословную.
...
6. Замена части количества имеет явную область действия.
...
Основания: [IPS-A05], [IPS-A13], [IPS-M03], [IL-PLM §5], [IL-ADR ADR-052]. Расшифровка — в [реестре источников](../knowledge/15-source-register.md).

...
Пользователь не обязан переходить в отдельную техническую программу, чтобы согласовать заказ или отправить его в цех. Рабочее место принимающего приложения показывает предметные действия, а общие средства Interlink выполняют согласованные операции с данными. Где размещены рабочие места и какие функции уже есть в существующей PLM, определяется интеграцией предприятия.
...
[Справочные таблицы](../tabular-data/01-reference-tables.md) позволяют искать сортамент по размерам и материалу, [многомерные таблицы](../tabular-data/02-multidimensional-tables.md) — читать норму или испытание по нескольким условиям, [координатные карты](../tabular-data/03-coordinate-maps.md) — получать кандидатов изделия либо покрытия. Таблица не выбирает за технолога, если результат неоднозначен. Отсутствующая точка не становится нулевой нормой. Для выпуска сохраняются точная редакция данных, использованные значения, единицы, формулы и округления.
...
| Функция | Владелец и проверяемая граница |
|---|---|
| Карточки ПЗ и ПВ, задания и сроки согласования | Принимающий продукт; один и тот же фундамент может обслуживать разные процессы |
| Полномочия на действие и чтение | Принимающий продукт и корпоративная система доступа; обязательные проверки действуют и при обходе экрана |
| Рабочее сохранение и совместная публикация выбранных изменений | Общие средства Interlink; опубликованные состояния не редактируются задним числом |
| Подбор версий, состава и объяснение результата | Общий механизм и явно подключённые предметные правила; полный допуск не выводится из успеха одной проверки |
| Точное основание передачи | Interlink по разрешённой команде; обязательные дополнительные ресурсы удерживаются по профилю предприятия |
| Юридический комплект и подписи | Принимающий продукт или корпоративный архив; один снимок не доказывает правовую достаточность |
| Доставка, ответы и коррекция внешнего задания | Средство интеграции по договору получателя; устойчивый номер сам не исключает дубликат |
| Запасы, планирование и принятые факты исполнения | ERP/MES или соответствующий предметный модуль |

Такое распределение не запрещает хранить журнал интеграции или производственные факты в модели поверх Interlink. Оно запрещает скрыто смешивать их с инженерным состоянием и приписывать ядру процесс предприятия.
...
| Подготовленный номер показан как публикация | Фиксация ещё не подтверждена | Не отправлять до подтверждения; при потере ответа установить действительный исход |
| Из десяти изделий хотят выдать пять без распределения | Не определены самостоятельная область и количественное основание | Создать область первой очереди и проверить распределение |
...
Связанные документы: [Общая модель и границы заказного контура](01-order-domain-and-boundaries.md), [Жизненный цикл живого заказа](04-live-order-lifecycle.md), [Точный состав и публикации заказа](05-exact-publication.md), [Формирование производственного заказа](06-production-order-resolution.md), [Экземпляры, партии и фактический состав](09-units-lots-and-as-built.md), [MRP, ERP/MES и длительные операции](11-integration-and-mrp.md).

## Основания и проверка источников

Основания: [IPS-A13], [IL-HOST], [IL-ORDERS], [IL-KERNEL], [IL-SC], [IL-TD], [IL-AI], [IL-DATA-MODEL]. Расшифровка и статус материалов — в [реестре источников](../knowledge/15-source-register.md).

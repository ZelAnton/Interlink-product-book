...
## Словарная близость не является совместимостью

[Понятия и их сопоставления](../object-foundation/06-domain-concepts.md) полезны для поиска кандидатов: например, найти инструкции или изделия с разными принятыми названиями. Они не доказывают допуск оборудования, взаимозаменяемость уплотнений или совместимость комплекта управления.

Для положительного решения нужны собственные условия и свидетельства совместимости. Если правило использует значение понятия, оно удерживает точный использованный смысл и существенные связи. Правка словаря не пересчитывает старое одобрение и не разрешает новую комбинацию задним числом. [Общий адрес элемента](../object-foundation/04-elements-and-concepts.md) не ослабляет эти предметные требования.
...
Обычная [карта сопоставлений](../tabular-data/03-coordinate-maps.md) не считает пустую клетку ни разрешением, ни запретом. Более сильный вывод возможен только тогда, когда объявлен полный перечень для известной области и проверено всё его авторитетное содержание.
...
Связанные материалы: [карты сопоставлений](../tabular-data/03-coordinate-maps.md), [полные сетки](../tabular-data/02-multidimensional-tables.md), [допустимые замены](01-allowed-replacements.md), [условия присутствия позиций](../version-selection/13-configurator-and-presence.md), [заказное конфигурирование](../orders/03-configuration-and-exact-product.md).

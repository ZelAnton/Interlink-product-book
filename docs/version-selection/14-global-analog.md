...
Схема раскрывает работу при включённом рассмотрении аналогов. В режиме «не подбирать» исходный объект не заменяется; просмотр каталога, если он нужен, запускается отдельно и сам не меняет результат состава.
...
Этот пример также показывает границу глобального аналога. Если замена одного двигателя всегда требует добавить две другие позиции, одной связи «объект заменяет объект» может быть недостаточно. Тогда решение оформляется как [групповая замена](15-nm-substitutions.md).
...
На три месяца утверждён подшипниковый узел другого поставщика для выполнения крупного заказа. В целевом правиле Interlink он назначен предпочтительным только в пределах конкретного проекта и периода поставки. Это пример дополнительного контекстного правила, а не утверждение о наличии в IPS числового приоритета.

При формировании заказа проекта система предлагает временный аналог первым. Для других проектов продолжает использоваться исходный объект. После окончания периода эта связь больше не подтверждает действующий аналог в режиме, который требует действия на дату запроса. Отдельный режим «подобрать один» сохраняет свою исходную семантику: результат подбора сам по себе не разрешает новое применение после истечения допуска. Ранее зафиксированные заказы сохраняют выбранный аналог и основание его применения; возможность новой производственной или сервисной операции проверяется отдельно по актуальным ограничениям.
...
- Что показывает используемая установка в режиме «подобрать действующий» при двух действующих аналогах, и как режим «подобрать один» выбирает результат при одинаковых признаках приоритета или отсутствии действующего аналога?
...
3. Три режима IPS дают различимые результаты. Множественность в режиме только однозначного действующего аналога не скрывается выбором первого; режим «подобрать один» воспроизводит подтверждённый исходный порядок, а не подменяется показом списка.
...
10. Показ всех кандидатов и рекомендация не выдаются за исходные режимы IPS или за уже применённое решение; дата запроса и приоритетный признак сохраняют исходный смысл.
...
- [Допустимые замены: от разрешения до применения](../substitution-and-compatibility/01-allowed-replacements.md)
...
- [Настраиваемое правило подбора версий](10-custom-selection-rule.md)
- [Точный состав производственного заказа](16-order-and-snapshot.md)

## Основания и границы подтверждения

Основания: [IPS-A04], [IL-KERNEL], [IL-VERS-SPEC], [IL-DATA-MODEL], [IL-PLAN-V2], [IL-SC]. Расшифровка и статус источников — в [реестре источников](../knowledge/15-source-register.md).

Материалы IPS подтверждают исходную потребность и известные варианты поведения; настройки конкретной установки проверяются при переносе. Нормативные документы Interlink задают целевой договор, но сами по себе не доказывают готовность его исполнения.

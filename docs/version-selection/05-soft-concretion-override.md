...
Поэтому в Interlink предусмотрена отдельная [полная мягкая конкретизация](20-durable-soft-concretion.md). Эта страница описывает более узкую возможность: временно предпочесть инженерную версию для испытания варианта, не объявляя это постоянным решением.
...
## Как переносить старые действия без потери смысла

Для каждой команды IPS нужно установить не только название, но и адрес, срок жизни и последствия. Если выбранная версия сохранялась на связи, участвовала в отчётах после завершения редактирования и могла восстановиться по жизненному циклу, её следует сопоставлять с полной мягкой конкретизацией. Называть такой выбор временным только из-за того, что его можно снять, неверно.

Если настройка действительно служила одному сеансу сравнения, нужно перенести объявленную область временного предпочтения. Если решение использовалось всей группой конструкторов в нескольких последовательных изменениях, следует проверить долговечный контекст. Перенос в один общий признак «предпочтительная версия объекта» потерял бы различия между позициями и независимыми работами.

Отдельно сохраняется смысл массовых команд. Список изменённых позиций, пропущенные недоступные владельцы и причина пропуска входят в результат. Число успешно обработанных строк не доказывает, что изменён весь заявленный состав.

## Расчёты, отчёты и выгрузки с временным выбором

Проверка нового подшипника влияет не только на геометрию, но и на ведомость покупных изделий, оценку стоимости, монтажные указания и испытательный режим. Все производные результаты должны получать ту же явно выбранную область проработки. Нельзя рассчитать массу по версии 03, а в ведомость незаметно включить серийную 02.

При сохранении такого исследования удерживаются точные использованные материалы и обозначение допущения. Если исходником была рабочая копия, создаётся пригодное для последующего чтения неизменяемое сохранение, а не только запоминается её изменяемое имя. Это не объявляет материал производственно опубликованным.

Разрешение экспортировать исследовательский результат задаёт прикладной процесс. Получатель должен видеть его назначение и ограничения. Передача файла наружу сама по себе не делает временный выбор официальным решением заказа.
...
- [Полная мягкая конкретизация](20-durable-soft-concretion.md)
- [Контексты и совместная работа](21-selection-contexts.md)
- [Жёсткие и точные привязки](02-hard-concretion-pin.md)
- [Применяемость](03-effectivity-series-date.md)

## Основания и границы источников

Основания: [IL-SOFT], [IL-IPS-COVERAGE], [IPS-A04], [IPS-A05], [IL-VERS-CONCEPTS], [IL-KERNEL], [IL-VERS-SPEC], [IL-DATA-MODEL], [IL-PLAN-V2]. Расшифровка и статус источников — в [реестре источников](../knowledge/15-source-register.md).

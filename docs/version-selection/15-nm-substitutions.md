...
Вариант с гидроблоком разрешён только начиная с определённой серии прессов, потому что у старой рамы недостаточно места. Выбор по серийному номеру относится к варианту узла. После выбора для гидроблока, крепежа и рукава подбираются версии по явно заданным условиям заказа. Если используется дата применяемости, её предметный смысл определяется отдельно: дата создания заказа не подставляется вместо неё автоматически.
...
```mermaid
sequenceDiagram
    participant E as Инженер комплектации
    participant W as Рабочее место заказа
    E->>W: Привязать библиотечный комплект к позициям
    W-->>E: Исходные участники, количества и условия
    E->>W: Выбрать замену для данного заказа
    W-->>E: Полное изменение состава и результаты проверки
    E->>W: Подтвердить применение выбранного плана
    W-->>E: Весь комплект применён либо весь план отклонён
```

Это путь пользователя, дополняющий схему подбора версий. Перед подтверждением он видит не одну «новую деталь», а полный результат: что исчезнет, что добавится и что должно сохраниться для других выбранных решений. Ошибка одной обязательной части не оставляет заказ с половиной замены.
...
- [Глобальный аналог объекта](14-global-analog.md)
...
Основания: [IPS-A05], [IL-KERNEL], [IL-VERS-SPEC], [IL-DATA-MODEL], [IL-PLAN-V2], [IL-SC]. Расшифровка и статус источников — в [реестре источников](../knowledge/15-source-register.md).

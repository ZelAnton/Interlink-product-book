...
## Правило по умолчанию не должно быть скрытой переменной

В IPS режим последних версий часто используется, когда пользователь не выбрал другой способ фильтрации. При переносе нужно проверить, в каких модулях действует именно такое умолчание: дерево состава, отчёт, поиск или производственный расчёт могут отличаться.

В Interlink умолчание входит в явный профиль рабочего места. Даже если пользователь не выбирает его вручную каждый раз, запущенный расчёт должен знать конкретное правило и область допустимых версий. Переключение окна не изменяет уже начатое формирование заказа.

При отсутствии необходимого назначения профиля программа не должна выбирать между «последняя вообще» и «последняя выпущенная» по догадке. Для производства такое различие является предметным требованием.

## Пример 5. Последняя опубликованная и последняя выпущенная — разные результаты

У технологического процесса версия 03 введена в производство. Версия 04 уже имеет опубликованное неизменяемое содержание, но ещё проходит согласование. Рабочие материалы и опубликованное содержание — не синонимы стадий жизненного цикла.

Разрешённый инженерный профиль может выбрать 04 для проверки. Производственный профиль выбирает 03, если она остаётся допустимой. В обоих случаях система читает официально сохранённые данные, но цели применения различаются.

Если 04 затем проходит требуемый переход и допускается к производству, новый живой расчёт может выбрать её. Сохранённый снимок задания, ранее сформированный с 03, продолжает ссылаться на прежний точный комплект. Простой факт повышения готовности не пересчитывает старый документ.
...
- [Назначенная основная версия](06-base-version-current.md)
- [Содержание и итерации версии](22-iterations-and-checkpoints.md)
...
Основания: [IPS-A04], [IPS-K03], [IL-QUERY], [IL-KERNEL], [IL-VERS-SPEC], [IL-DATA-MODEL], [IL-PLAN-V2]. Расшифровка и статус источников — в [реестре источников](../knowledge/15-source-register.md).

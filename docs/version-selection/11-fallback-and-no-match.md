...
Строгий режим «только жёсткие привязки» принимает привязку к инженерной версии либо к точному состоянию. Более строгий режим «только точные состояния» требует именно неизменяемого опубликованного содержания: одного номера инженерной версии недостаточно. В обоих режимах отсутствие нужного вида фиксации останавливает подбор до более слабых источников и резерва. Рабочий контекст, применяемость и мягкая конкретизация не восполняют отсутствующее обязательное указание незаметно.

Отсутствие фиксации отличается от ошибочной заданной ссылки. Если она повреждена, принадлежит другому объекту или ведёт к недоступным данным, система показывает допустимую по правам диагностику целостности или доступа. Нельзя удалить смысл ошибки и продолжить подбор так, будто пользователь вообще ничего не закрепил.

Если инженерная версия уже выбрана, но у неё нет требуемого содержания, проблема относится к содержанию этой версии. Например, для выбранной версии 07 ещё нет опубликованных материалов, необходимых производственной передаче. Требуется подготовить и опубликовать их либо явно выбрать другой допустимый режим работы; система не начинает резервный поиск версии 06. Аналогично, режим, несовместимый с заданной точной или рабочей привязкой, требует исправления условий, а не молчаливого ослабления привязки.

При этом обычное чтение рабочих материалов не обязано требовать конкретную рабочую копию. Если она не задана явной привязкой, а выбранный режим допускает при отсутствии рабочих материалов читать действующее опубликованное состояние, система может прочитать его у **той же инженерной версии 07**. Это разрешённый способ чтения содержания, а не резервный выбор другой версии. Явно закреплённую рабочую копию нельзя таким способом незаметно заменить опубликованными данными.
...
9. Отсутствие нужного вида строгой фиксации, ошибка заданной ссылки и отсутствие содержания уже выбранной версии имеют разные объяснения; ни одно не запускает скрытый выбор другой версии.

## Как превратить предупреждение в решаемую рабочую задачу

Одного жёлтого значка недостаточно. Специалисту нужны невыполненное требование, затронутая позиция и путь, основание резерва, показанные данные и перечень запрещённых дальнейших действий. Подробность кандидатов определяется доступом, а не желанием объяснить всё любой ценой.

Затем рабочее место должно предложить допустимый следующий шаг. Недостающую дату заказа уточняет ответственный за заказ. Отсутствующую утверждённую версию готовит владелец документа. Конфликт диапазонов исправляет владелец применяемости. Неподдерживаемый вид проверки передаётся методисту или команде продукта; его нельзя устранить назначением случайной версии.

Если причина связана с предметными данными, после их исправления выполняется повторный расчёт. После технического сбоя восстанавливают корректное выполнение; менять версию или условие заказа ради обхода сбоя не нужно. Предыдущее объяснение сохраняется как основание работы. Благодаря этому видно, что проблему устранили новой квалификацией уплотнения, а не просто скрыли предупреждение в отчёте.

Для предметного препятствия путь пользователя отличается от самого алгоритма резервного выбора:

```mermaid
flowchart TD
    A["Сохранить позицию, невыполненное требование и причину"] --> B{"Какой способ устранения разрешён?"}
    B -->|Исправить исходные данные| C["Владелец исправляет данные и основание"]
    B -->|Допустимо отдельное решение заказа| D["Получить решение с областью и полномочиями"]
    B -->|Ни один| E["Оставить препятствие открытым"]
    C --> F["Повторить нужные проверки"]
    D --> F
    F --> G{"Обязательные условия выполнены?"}
    G -->|Да| H["Подготовить допустимый точный результат передачи"]
    G -->|Нет| E
```

Резерв не является решением заказа. В приведённых исследовательских примерах он позволяет показать условный вариант, сохранив исходную проблему; в общем случае допустимость резерва и дальнейших действий задаёт явный профиль операции. Отдельное предметное решение требует собственных полномочий и не может отменить неотменяемый запрет. Если обязательное препятствие остаётся открытым, можно продолжать разрешённый анализ, но нельзя объявлять готовой производственную передачу.

## Где условный результат может сохраняться

В протоколе предварительной оценки можно сохранить версию, использованную как допущение, вместе с её точным содержанием и ограничениями. Если исходником были незавершённые материалы, удерживается неизменяемое сохранение для анализа; это не фиктивная публикация инженерной версии.

Для нового производственного результата требуется допустимый точный комплект. Когда процесс разрешает предметное исключение, сохраняется отдельное решение в его конкретной области. Точная фиксация сама по себе не превращает запрещённое решение в разрешённое: она отвечает за неизменность адреса, а не за право применения.

Плановый заказ, исследовательский отчёт и официальный комплект передачи могут иметь разные требования полноты. Поэтому запрет «нельзя сохранить вообще ничего, пока есть предупреждение» был бы столь же неверен, как разрешение молча передать неполный комплект в производство.
...
- [Глобальные аналоги](14-global-analog.md)
- [Групповые замены](15-nm-substitutions.md)
- [Точные привязки](02-hard-concretion-pin.md)
- [Производственный состав заказа](16-order-and-snapshot.md)

## Основания и границы источников

Основания: [IPS-A04], [IPS-K03], [IL-SC], [IL-SOFT], [IL-IPS-COVERAGE], [IL-KERNEL], [IL-VERS-SPEC], [IL-DATA-MODEL], [IL-PLAN-V2]. Расшифровка и статус источников — в [реестре источников](../knowledge/15-source-register.md).

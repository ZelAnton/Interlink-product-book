...
sequenceDiagram
    ...
    participant W as Рабочее место
    participant R as Согласующий
    E->>W: Сохранить правки рабочей копии
    W-->>E: Работа сохранена, публикации нет
    E->>W: Подготовить выбранный комплект
    W->>R: Показать точное содержание и проверки
    R-->>W: Решение об этом содержании
    E->>W: Выполнить разрешённую публикацию
    W-->>E: Опубликованный комплект либо объяснимый отказ
```

Здесь показано участие людей, а не только технический порядок проверки. Согласующий принимает решение о конкретном подготовленном результате. Инженер не может подменить его последующей правкой, а сохранение рабочей копии не является скрытым приказом публиковать документ.
...
- [Работа, контекст и пакет изменения](04-editing-context-changeset.md)
...
Основания: [IPS-A04], [IPS-K04], [IL-KERNEL], [IL-VERS-SPEC], [IL-DATA-MODEL], [IL-PLAN-V2]. Расшифровка и статус источников — в [реестре источников](../knowledge/15-source-register.md).

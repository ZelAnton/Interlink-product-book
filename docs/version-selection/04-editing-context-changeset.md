...
В IPS контекст может быть связан не только с обычной проработкой, но и с извещением, комплектом извещений, записью журнала изменений или заказом. Связанные контексты дают общий набор назначений; разные версии одного объекта в такой группе требуют разбора, а не выбора последнего открытого окна. В Interlink этот предметный смысл сохраняется в самостоятельных контекстах подбора.

При переносе отдельно проверяется, как исходная установка хранит активный контекст и автопополнение. Практическая потребность остаётся прежней: два окна и фоновая задача не должны непредсказуемо менять условия друг друга, а отчёт должен позволять установить использованный набор данных. Это требование к проверке конкретной установки, а не утверждение, что любой интерфейс IPS обязательно работает ошибочно.
...
Извещение объясняет, зачем изменение проводится и на каком основании; процесс согласования назначает задания, рецензентов и решения; пакет определяет публикуемый предметный результат; снимок сохраняет точное содержание передачи. Рабочее место может объединить эти вещи визуально, но одинаковый номер извещения не делает их одной записью и не отменяет разные сроки жизни.

```mermaid
flowchart TD
    A["Контекст конструкторов"] --> C["Общий будущий состав для чтения"]
    B["Контекст технологов"] --> C
    A --> D["Отдельное решение о публикации механики"]
    B --> E["Отдельное решение о публикации технологии"]
    D --> F["При необходимости общий неделимый комплект"]
    E --> F
```

Схема показывает две независимые связи: совместно читать можно раньше, чем совместно публиковать. Общий комплект нужен только при предметном требовании неделимого результата; его не создаёт сам факт объединения контекстов.
...
Кроме деталей требуется рабочая копия редуктора версии 07 с изменёнными позициями и соответствующая спецификация версии 07. Иначе геометрия могла бы стать новой, а состав и документ остаться прежними. В обычном производственном представлении до разрешённой публикации сохраняются ранее выпущенные вал версии 03, крышка версии 05 и состав редуктора версии 06. Участники будущего изменения видят новое содержание через свой явный контекст и могут проверить всю цепочку от детали до документа.

Все запланированные результаты видны вместе. Проверка обнаруживает, что новая крышка не подходит к старому валу. Вал и крышку нельзя публиковать независимо: они входят в один неделимый пакет. Технологическую карту, изменённый состав и спецификацию процесс предприятия также требует включить в передачу. Подшипнику не нужна новая версия: его существующее точное состояние выступает основанием.
...
- [Контексты подбора и совместная работа](21-selection-contexts.md)
- [Мягкая конкретизация позиции](20-durable-soft-concretion.md)
...
Основания: [IPS-A05], [IPS-K04], [IL-VERS-CONCEPTS], [IL-KERNEL], [IL-VERS-SPEC], [IL-DATA-MODEL], [IL-PLAN-V2]. Расшифровка и статус источников — в [реестре источников](../knowledge/15-source-register.md).

...
Редакция: **6 сентября 2026 года**. Статус: продуктовое объяснение принятого целевого договора; сквозное исполнение ещё требует реализации и приёмки.
...
## На что агент может ссылаться и что обязан сохранять

[Адресуемые элементы](../object-foundation/04-elements-and-concepts.md) дают общий способ указать определение, инструкцию, конкретный шаг или результат. Но процедура, агент как исполнитель, его запуск, отдельная попытка и делегирование остаются разными предметами. [Применение процедуры](../object-foundation/05-instructions-and-execution.md) ещё не доказывает её выполнение.

Для разбора решения сохраняются фактические входы и показанные агенту сведения, нужные исторические состояния и результаты действий. Общая ссылка не замыкает всю историю автоматически. [Смысловой поиск](../object-foundation/06-domain-concepts.md) помогает найти кандидатов, но не выдаёт полномочия и не подтверждает совместимость. Ручное и автономное выполнение сохраняют свои основания доступа и ответственности.
...
## Состояние реализации и ближайшая проверка

Проверка агентских оснований входит в раннее испытание фундамента — этап S1, то есть небольшой сквозной опыт до закрепления следующего устройства модели. Она не отложена целиком до появления готового Alvatune или до завершения опытного проекта Interlink. Вместо настоящей языковой модели достаточно управляемого тестового исполнителя: важно проверить данные и последствия, а не качество формулировки ответа.

В опыте используются две рабочие области, рабочий чертёж насоса, замечание качества и внешний каталог. Один участник меняет содержание, пока другой собирает сведения. До передачи должны долговечно сохраниться разрешённое наблюдение и фактически подготовленный пакет; сбой до этого момента не должен оставлять выданные сведения без основания. Проверяется также, что остановленный после нескольких страниц просмотр не записан как полный.

Следующие этапы переносят доказанные гарантии в общие компоненты, расширяют варианты чтения и проводят нагрузочную проверку. Историческое чтение после обновления модели должно сохранять прежнее понимание единиц, обозначений, преобразований и редакции памяти. Новая программа, которая умеет открыть старую запись, но интерпретирует её новым несовместимым правилом, не проходит такую приёмку.

Сохранение контекста, состояние шага и контроль исполнения подробнее связаны в [границе Interlink и управляющего приложения](03-alvatune-and-host-boundary.md). Конкретный состав рабочих мест, сроки хранения и политика внешних получателей выбираются для внедрения; они не являются уже готовыми возможностями платформы.

Продолжение: [безопасные действия и восстановление](02-safe-actions-and-recovery.md), [производительность при агентской нагрузке](../data-structure/06-performance-and-agent-load.md), [цифровые двойники](../object-foundation/03-digital-twins-and-domain-extension.md), [текущее состояние и план](../knowledge/02-current-state-and-roadmap.md).

## Основания

Смысл проверен по источникам IL-AI, IL-NEUTRAL, IL-FOUNDATION, IL-DATA-MODEL и IL-PLAN-V2. Их приоритет, область и состояние указаны в [реестре источников](../knowledge/15-source-register.md). Инженерные ограничения согласуются с IL-KERNEL; эта глава не вводит отдельный механизм выбора версий или прав для агентов.

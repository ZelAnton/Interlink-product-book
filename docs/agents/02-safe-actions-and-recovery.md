...
Редакция: **6 сентября 2026 года**. Статус: продуктовое объяснение целевого договора, а не описание законченного агентского рабочего места.
...
При этом не вся случайно прочитанная информация должна блокировать работу. Исправление телефона поставщика может не иметь отношения к выпуску редуктора. Существенные зависимости задаются предметным правилом, чтобы защита была осмысленной, а не запрещала любые параллельные изменения.
...
## Состояние реализации и ранние испытания

Основная проверка безопасного действия запланирована внутри раннего этапа S1, до закрепления зависимой формы будущего ядра. Для этого не требуется промышленный агентский продукт: тестовый исполнитель подготавливает исправление насоса, человек или проверочная процедура принимает решение, а управляемые сбои разрывают связь в заранее известных местах.

Опыт должен подтвердить один локальный эффект при двух попытках, отказ при изменении смысла под прежним обозначением операции, сохранение квитанции после потери ответа и запрет запоздалой записи от прежнего исполнителя. Отдельно проверяется обязательное условие «нет открытых замечаний»: новая запись между анализом и применением должна влиять на результат. Наличие текстового журнала или одобренного плана без фактической проверки записи недостаточно.

Последующие этапы расширяют проверку на настоящие прикладные соединители, массовые операции и нагрузку. Полный промышленный обмен не считается готовым по успеху локального опыта. Внешние ограничения должны быть известны до включения автоматического повтора: где можно сверить прежний результат, где допустима отдельная исправляющая операция, а где нужен ответственный человек.

Порядок работы управляющего приложения, его ожидания и вмешательство оператора раскрыты в [границе Interlink и Alvatune](03-alvatune-and-host-boundary.md). Подтверждение этих общих гарантий не требует откладывать обязательное покрытие IPS и не заменяет его самостоятельную предметную приёмку.

Продолжение: [агент и фактический контекст](01-agents-and-traceability.md), [пакет изменения](../changes/README.md), [допустимые замены](../substitution-and-compatibility/01-allowed-replacements.md), [совместимость](../substitution-and-compatibility/02-compatibility-matrices.md), [план развития](../knowledge/02-current-state-and-roadmap.md).

## Основания

Основания: IL-AI, IL-KERNEL, IL-FOUNDATION, IL-NEUTRAL, IL-DATA-MODEL и IL-PLAN-V2 в [реестре источников](../knowledge/15-source-register.md). Они определяют целевые гарантии; наличие описания квитанции или инструмента не подтверждает готовность исполнения.

...
## Общий адрес не означает общую таблицу для всего

[Адресуемый элемент](../object-foundation/04-elements-and-concepts.md) — договор ссылки, а не требование поместить все определения, объекты и переходы в одну таблицу. Типизированные формы хранения сохраняются. Локальный переход может храниться вместе со структурой маршрута и проверяться через владельца; число не становится самостоятельным объектом лишь ради общей адресации.

В раннем плане предстоит подтвердить целостность ссылок, принадлежность частей, сохранение точного прошлого и защиту от обходных записей. Наличие адреса в каталоге или поисковом индексе не доказывает наличие исторического содержания. Стоимость этих проверок и индексации измеряется отдельно: новый уровень обобщения сам по себе не доказывает рост производительности.
...
Справочник типоразмеров не обязательно превращать в тысячи самостоятельных инженерных деталей. В план добавлены [справочные таблицы](../tabular-data/01-reference-tables.md), [полные многомерные таблицы](../tabular-data/02-multidimensional-tables.md) и [координатные карты](../tabular-data/03-coordinate-maps.md). Их значения также должны иметь типизированное индексируемое хранение.
...
Физическое построение относится к проверяемой незавершённой работе над хранилищем; его наличие не является свидетельством промышленного внедрения. Минимальные проверки общего реестра, принадлежности ссылок и новых табличных форм нужны до реализации зависимой записи, а не откладываются целиком на время после опытного этапа.
...
Актуальные уточнения архитектуры: [IL-KERNEL], [IL-STORAGE], [IL-NEUTRAL], [IL-TD], [IL-DATA-MODEL], [IL-PLAN-V2]. При расхождении с ранними описаниями действуют пересмотренные специализированные договоры. Состояние реализации относится к срезу документации от 6 сентября 2026 года.

Исходные предметные и исследовательские основания: [IL-DB §1–§9], [IL-META §2–§6], [IL-POC §6–§8], [IPS-DB01], [IPS-CRITIQUE]. Расшифровка — в [реестре источников](../knowledge/15-source-register.md).

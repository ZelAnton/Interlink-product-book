...
- количество объектов, инженерных версий, опубликованных состояний, рабочих копий, связей и значений по согласованным категориям;
...
- обратное применение;
- жёсткая конкретизация инженерной версии и отдельная фиксация точного опубликованного состояния;
...
- выбранные объекты, инженерные версии и точное содержимое там, где оно обязательно;
- количества;
- порядок;
- причины выбора;
...
Для перехода с IPS отдельно проверяются последние изменения после исходного среза и передача единственного права записи. План возврата должен учитывать данные, которые пользователи успели создать в Interlink. Восстановление работоспособности после сбоя установки и возврат предметной области обратно в IPS — разные процедуры. Простое включение старой копии IPS не восстановит новые пользовательские данные: нужен доказанный обратный перенос, запрет новых записей в контрольный период либо заранее согласованная точка, после которой применяется другой способ исправления.
...
- неизменяемое опубликованное содержание инженерных версий;
...
- [Предметный язык и редакции модели](03-dsl-product-language.md)
- [Выпуск и установка проверенной модели](17-model-release-and-installation.md)
- [Статусы и риски](13-status-risks-and-acceptance.md)
- [Многомерные таблицы](../tabular-data/02-multidimensional-tables.md)
- [Соответствия по нескольким осям](../tabular-data/03-coordinate-maps.md)
- [Матрицы совместимости](../substitution-and-compatibility/02-compatibility-matrices.md)

## Основания

[IL-MIG], [IL-DB], [IL-POC], [IL-VERS-IPS], [IPS-A04], [IPS-DB01], [IL-KERNEL], [IL-STORAGE], [IL-TD], [IL-SC], [IL-DATA-MODEL], [IL-PLAN-V2]. Расшифровка и границы достоверности — в [реестре источников](../knowledge/15-source-register.md).

Источники IPS дают исходные сценарии подбора и частичную реконструкцию хранения, но не готовую программу промышленного переноса. Эквивалентность и нагрузочные показатели проверяются на выбранной установке. Актуальные контракты Interlink задают целевой процесс и защиту истории; описание испытания в плане ещё не означает, что оно выполнено.

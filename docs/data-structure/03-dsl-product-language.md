...
## Где заканчивается язык модели и начинаются предметные данные

Разделение [типа, инструкции и применения](../object-foundation/05-instructions-and-execution.md) снимает необходимость описывать каждый новый маршрут как новый структурный тип. Если установленная форма уже допускает инструмент, режим и поверхность, технолог создаёт инструкцию и её применения как данные. Если новое требование невозможно выразить в разрешённой форме, требуется изменение модели с проверкой последствий.

Предметное понятие или синоним обычно меняются в подключённом словаре. Они не могут добавлять обязательный атрибут или отменять проверку установленного типа. Общая ссылка на определение нужна для чтения и обсуждения, а не для обходного редактирования модели.

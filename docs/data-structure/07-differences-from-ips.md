...
| Целостность | Значительная часть поддерживается ядром и процедурами | Внешние ключи и ограничения БД вместе с обязательными проверками записи | Меньше висячих данных и явная граница гарантии каждого слоя |
...
Ссылка на объект, жёсткая конкретизация инженерной версии и точная ссылка на опубликованное содержание имеют разный смысл. Interlink сохраняет пользовательскую потребность IPS «не переключаться на другую версию», но не выдаёт её за более сильную гарантию «читать неизменные байты и значения». Для исторического комплекта нужна именно последняя гарантия. Подробнее — [версии и состав](../version-selection/README.md).
...
IPS различает ссылку на объект и версию отдельными механизмами — это сохраняется. В целевом Interlink ссылка имеет проверяемый вид: на устойчивый материал, выбранную инженерную версию или точное содержание. Ссылочная целостность обеспечивается предметной таблицей либо общим структурным реестром с проверкой принадлежности. Существующий, но чужой объект не должен подходить только потому, что найден его идентификатор. Допустимость конкретного типа и прочие предметные ограничения дополнительно проверяет общий путь записи; это не обещание, что любой сложный запрет выражается одним ограничением базы.

...
### Что означает «материализация»

В документах встречаются два разных результата. **Физическая материализация модели** — создание настоящих таблиц, столбцов, ссылок и индексов. **Превращение результата чтения в типизированные данные приложения** — получение понятной приложению записи из уже прочитанных строк. Наличие второго механизма не доказывает готовность первого и тем более не доказывает безопасную установку промышленного обновления.

На текущем срезе уже есть два уровня генерации прикладных средств, сравнение моделей и проверки каталога. Формирование физической схемы развивается в незавершённой работе. Следующий рубеж — согласованный путь по новой архитектуре: одна проверенная поставка, реальные данные, правильное чтение и изменение, публикация и восстановление после сбоя. Старые тесты полезны, но сами по себе не подтверждают этот новый сквозной договор.
...
Для количественного сравнения с IPS нужен отдельный согласованный эталон: выбранная установка либо сохранённые проверяемые измерения, одинаковый предметный результат, сопоставимые объёмы и явно описанное оборудование. Нельзя сравнивать быстрый поиск без проверки доступа с полным пользовательским запросом IPS или маленький демонстрационный состав с многолетней базой предприятия. Пока такого сравнения нет, корректно говорить об ожидаемом снижении конкретных издержек, а не о доказанном превосходстве.
...
3. Совпадение контрольных составов и обратной применяемости, включая жёсткую и мягкую конкретизацию, исключения и установленный в IPS порядок выбора.
...
8. Проверенный отказ от переключения до его завершения; после допуска новой записи — отдельный план восстановления, учитывающий уже появившиеся данные.
...
Новые области нельзя отложить целиком до появления отраслевого продукта: иначе в ядре успеют закрепиться неудобные ограничения. Поэтому первые архитектурные этапы и узкий опытный образец должны заранее подтвердить, что адрес склада меняется без инженерной версии, акт измерения исправляется отдельным фактом, а таблица из миллиона чисел не создаёт миллион версионных объектов.
...
Актуальный замысел, договоры и последовательность проверки: [IL-KERNEL], [IL-STORAGE], [IL-FOUNDATION], [IL-NEUTRAL], [IL-AI], [IL-TD], [IL-SC], [IL-DATA-MODEL], [IL-PLAN-V2]. Предметные и исторические основания этой страницы: [IL-POC], [IL-ADR], [IL-DB], [IL-MIG], [IL-HOST], [IL-DT-OPPORTUNITY], [IL-DT-CONCLUSION], [ALV-ADR068], [ALV-AGENT], [IPS-CRITIQUE]. Расшифровка обозначений и пределы свидетельств — в [реестре источников](../knowledge/15-source-register.md).

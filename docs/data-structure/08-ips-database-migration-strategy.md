...
| Форма/обработчик | Хост или предметный модуль | Не переносить исполняемую логику как данные |
...
- функции хоста: формы, права, процессы, интеграции;
...
Локальный тип IPS имеет свои поля, форму и обработчик нумерации. Тип и данные переходят в модуль предприятия. Поля становятся типизированными; форма и нумерация реализуются хостом. Обработчик не копируется как непрозрачный скрипт.
...
- [DSL и устойчивые идентификаторы](03-dsl-product-language.md)

Основания: [IPS-A01], [IPS-A44], [IPS-DB01], [IPS-M02], [IL-MIG], [IL-DSL], [IL-ADR ADR-017, ADR-031, ADR-045]. Расшифровка — в [реестре источников](../knowledge/15-source-register.md).

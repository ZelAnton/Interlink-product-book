...
## Определение типа тоже можно адресовать, но не менять как обычную запись

Принятый договор [элементов](../object-foundation/04-elements-and-concepts.md) объединяет проверяемую ссылку на тип, предметные данные и объявленную часть. Он не объединяет их команды изменения. Замечание к обязательному полю типа может храниться отдельно; исправление самого определения проходит управляемый выпуск модели.

Тип «Инструкция обработки» задаёт форму данных. Инструкция для посадочной шейки — данные этой формы. Два применения в маршруте вала имеют свои параметры и устойчивые адреса, а не два новых типа. Классификация инструкции понятием «Чистовое точение» объясняет смысл, но не меняет её структуру и не запускает обработку. Подробнее — [процессы](../object-foundation/05-instructions-and-execution.md) и [словарь понятий](../object-foundation/06-domain-concepts.md).
...
Неверсионируемые типы уже выражаются в логической модели. Целевой фундамент подробнее разделяет оперативно изменяемые записи и факты, которые только добавляются. Например, телефон склада можно исправить с проверкой чужой параллельной правки, а протокол получения измерения сохраняется неизменным; исправление оформляется новым связанным фактом. Ни одно из этих действий не требует фиктивной инженерной версии.

Для физического мира также нужны периоды действия и история исправлений: где был установлен насос, кому принадлежал экземпляр, когда произошло событие и когда о нём узнали. Это самостоятельная предметная семантика поверх общего хранения записей и фактов, а не обязательная инженерная версия каждого события. Подробности развития — [цифровые двойники](11-digital-twin-roadmap.md).
...
Минимальные проверки новых способов записи, табличных форм и ссылок входят в раннее опытное подтверждение фундамента. Они нужны до закрепления зависимых механизмов, а не целиком после опытного этапа. Полноценные отраслевые рабочие места могут появляться позднее.
...
Актуальные уточнения архитектуры: [IL-KERNEL], [IL-STORAGE], [IL-NEUTRAL], [IL-TD], [IL-SC], [IL-DATA-MODEL], [IL-PLAN-V2]. При расхождении с ранними описаниями действуют пересмотренные специализированные договоры. Состояние реализации относится к срезу документации от 6 сентября 2026 года.

Исходные предметные и исследовательские основания: [IL-META], [IL-DB], [IL-ADR ADR-037–043, ADR-047, ADR-050–052], [IPS-A01], [IPS-A03]. Расшифровка — в [реестре источников](../knowledge/15-source-register.md).

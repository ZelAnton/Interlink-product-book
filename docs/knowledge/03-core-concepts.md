...
Инструкция обработки, её применение к поверхности и фактическая обработка вала различаются. Технолог меняет параметры применения в маршруте, а измерение относится к выполненной работе. Подробные примеры — в [элементах и адресации](../object-foundation/04-elements-and-concepts.md) и [инструкциях и выполнении](../object-foundation/05-instructions-and-execution.md). Ниже эти уточнения дополняют прежние механизмы, а не отменяют их.

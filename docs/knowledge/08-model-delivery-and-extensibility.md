...
[Предметная инструкция и её применение](../object-foundation/05-instructions-and-execution.md) обычно создаются как данные установленной формы. Новая обязательная структура вне разрешённых возможностей требует управляемого изменения модели. Словарный синоним не может обойти этот путь.

[Адресуемое определение](../object-foundation/04-elements-and-concepts.md) можно использовать как цель чтения или замечания. Но его точный исторический смысл требует удерживать не только поставленную модель, но и использованные редакции настроек и расширений. [Предметные понятия](../object-foundation/06-domain-concepts.md) помогают объяснять смысл, не изменяя обязательные правила через подпись.

...
| В составе нет допустимой версии | Полный снимок запрещён; проведение самого пакета проверяет все обязательные условия выбранного профиля и целостность зависимостей | Устранить причину либо подготовить отдельный допустимый план без снимка. Если снимок обязателен для уже согласованной общей операции, исключить его и провести остаток по прежнему решению нельзя |
...
## Исключение из пакета и отказ от рабочей копии

Рабочую копию существующей инженерной версии можно исключить из выбранного пакета, если оставшийся набор образует целостный допустимый план. Исключение само по себе не удаляет копию из рабочей области: конструктор может продолжать её подготовку для другого выпуска. Если точный состав пакета уже согласован, изменённый план требует новой подготовки и предусмотренного процессом решения.

Отказ от самой рабочей копии — отдельное действие. Перед ним проверяются зависимости и обязательства хранения. Сам объект, его инженерная версия и опубликованная история сохраняются; отказ от незавершённых правок не означает удаления уже официального результата.
...
Разовое решение этого заказа остаётся в его выбранной конфигурации и точном снимке. Оно не назначает двигатель основным для всей продукции и не меняет общую применяемость. Если замена должна распространяться на будущие заказы, отдельно изменяются общие инженерные данные или правила. Так срочная поставка не превращается незаметно в новую норму предприятия.
...
Связанные документы: [рабочее состояние и параллельность](03-working-state-visibility-and-concurrency.md), [предпросмотр и согласование](04-preview-validation-and-approval.md), [снимки и доказательства](07-snapshots-evidence-and-user-history.md), [итерации и восстановление](../version-selection/22-iterations-and-checkpoints.md), [мягкая конкретизация](../version-selection/20-durable-soft-concretion.md).

## Основания

[IL-KERNEL], [IL-CHANGES], [IL-AI], [IL-PLAN-V2], [IPS-A05]. Актуальные нормы Interlink определяют целевое поведение; источники IPS используются для сопоставления пользовательских сценариев, а не как доказательство готовности Interlink. Расшифровка и статус источников — в [реестре источников](../knowledge/15-source-register.md).

...
1. **Опубликованные данные.** Система выбирает инженерную версию и её содержание по явным условиям: основная версия, серия, дата, готовность и другие предметные правила. Опубликованное не обязательно означает самое новое или пригодное для любого заказа; основная версия может ещё не быть назначена.
...
```mermaid
flowchart TD
    A["Общее исходное состояние"] --> B["Работа сервиса"]
...
Отдельно существуют временное предпочтение для проработки и **мягкая конкретизация позиции** — сохраняемое предметное предпочтение с собственными условиями действия. Нельзя требовать удалить мягкую конкретизацию только потому, что закончился пакет. И нельзя автоматически превратить временный выбор двигателя для всей проработки в постоянные решения каждой позиции. Подробнее — [мягкая конкретизация](../version-selection/20-durable-soft-concretion.md).

### Во что может превратиться пробный выбор

Инженер временно смотрит редуктор с двигателем версии «Г». После сравнения он может закончить просмотр без изменения данных; сохранить назначение в самостоятельном контексте для дальнейшей работы; задать мягкую конкретизацию нужной позиции; жёстко закрепить инженерную версию либо точно зафиксировать её опубликованное состояние.

Это разные решения с разными владельцами и сроками действия. В подготовленном плане видно, что было лишь наблюдением, а что стало сохраняемым назначением. Неоднозначный пробный выбор не преобразуется в постоянное решение автоматически. Применяемость также задаётся по собственному предметному смыслу, а не используется как новое имя временного предпочтения.

При завершении работы заранее определяется судьба ссылок на рабочие копии. Неясное или несовместимое назначение вызывает отказ либо новую подготовку. Самостоятельный контекст и мягкая конкретизация не должны исчезать лишь из-за закрытия пакета.
...
Связанные документы: [рабочие области](10-workspaces-and-parallel-work.md), [контексты подбора](../version-selection/21-selection-contexts.md), [итерации](../version-selection/22-iterations-and-checkpoints.md), [проведение и восстановление](05-commit-cancel-and-recovery.md), [агенты и прослеживаемость](../agents/01-agents-and-traceability.md).
...
Основные источники: [IL-KERNEL], [IL-CHANGES], [IL-AI], [IL-PLAN-V2], [IPS-A04], [IPS-A05], [IPS-K04]. Расшифровка, статус и местонахождение — в [реестре источников](../knowledge/15-source-register.md).

...
[Адрес элемента](../object-foundation/04-elements-and-concepts.md) сам по себе ещё не сохраняет прошлое. Для перехода маршрута нужны точное содержание владельца и необходимая цепочка вложенности. Для оперативно изменяемых данных требуется согласованное сохранённое состояние. Для определения типа — использованная модель и существенные редакции настроек и расширений, а не только её нынешнее название.

Если решение зависело от [понятия и его связей](../object-foundation/06-domain-concepts.md), сохраняется именно использованный смысл. Новая инструкция или словарная редакция не переписывает прежнее выполнение. Общая ссылка также не сохраняет автоматически всё содержимое связанных живых карточек: состав точного основания задаётся явно. Недоступные материалы обозначаются как пробел, а не заменяются текущими.
...
После подтверждения публикации прикладная система может отправить получателю сообщение или представление со ссылкой на точный результат. Если передача обязательна для выпуска, её устойчивое намерение сохраняется вместе с публикацией. Сам внешний вызов выполняется после подтверждённого локального успеха.

Получатель может принять сообщение, отклонить его, принять только явно обозначенный объём либо не вернуть достоверного ответа. Это отдельные события, которые не переписывают снимок. Частичная внешняя приёмка не превращает полный исходный снимок в незаметно урезанный.

Если публикация подтверждена, её не повторяют из-за сбоя доставки. Продолжают передачу того же результата по доказанному договору защиты от повторного эффекта либо сверяют исход с получателем. Если неизвестна судьба самой публикации, сначала устанавливают результат той же логической операции. Новое обозначение не используется для обхода неизвестности.
...
Однако точность не повышает инженерную готовность: снимок опытного черновика не становится выпущенным документом. При восстановлении пользователь явно выбирает адресатов и объём переноса. Восстановление только в рабочие копии не меняет опубликованные состояния. Если нужен официальный выпуск, он проходит отдельную обычную подготовку и проверку. Подробнее — [итерации и восстановление](../version-selection/22-iterations-and-checkpoints.md).
...
После проведения извещения предприятие публикует базовый снимок конкретного редуктора РЦ-250 с заводским номером 145, для площадки Пермь и даты выпуска 12 мая. Он фиксирует инженерные версии и точные состояния корпуса, валов, подшипников и крышек. Другие условия требуют собственного явного основания публикации. Через месяц появляется улучшенное состояние той же версии манжеты. Текущий просмотр может выбрать его, но прежний снимок сохраняет исходную манжету даже без изменения её инженерного обозначения.

Снимок, подготовленный только для редуктора с заводским номером 145, не доказывает автоматически, что тот же состав действовал для всего диапазона 145–200. Такое утверждение требует отдельно проверенной области применяемости и отсутствия иных публикаций или исключений в этом диапазоне. В истории должны различаться серия изделия, конкретный заводской номер и область утверждаемого вывода.
...
Разовая замена относится к этому заказу и его публикации. Следующий независимый заказ продолжает использовать общие инженерные данные и свои условия выбора. Распространить решение на будущую продукцию можно отдельным управляемым изменением общих правил, а не правкой исторического снимка.
...
Для приёмки нужно проверить три границы: новое состояние не меняет старый снимок; аналитическая итерация не выдаётся за выпуск; разрешение и передача не выдаются за фактическую установку. Отдельные испытания должны доказать сохранность обязательных данных при миграции и сбоях и честную диагностику их недоступности.

Связанные документы: [проведение и восстановление](05-commit-cancel-and-recovery.md), [точный состав заказа](../version-selection/16-order-and-snapshot.md), [публикация заказа](../orders/05-exact-publication.md), [итерации и восстановление](../version-selection/22-iterations-and-checkpoints.md), [агенты и прослеживаемость](../agents/01-agents-and-traceability.md).

## Основания

[IL-KERNEL], [IL-CHANGES], [IL-AI], [IL-PLAN-V2], [IPS-A13]. Актуальные нормы Interlink определяют целевое поведение; источники IPS используются для сопоставления пользовательских сценариев, а не как доказательство готовности Interlink. Расшифровка и статус источников — в [реестре источников](../knowledge/15-source-register.md).

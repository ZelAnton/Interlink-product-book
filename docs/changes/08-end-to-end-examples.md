...
В самостоятельной операции окончательный итог подтверждает Interlink. При включении в общую операцию принимающего продукта промежуточная готовность Interlink ещё не означает официальный результат: ждут подтверждения всей операции. Неизвестный исход возможен в обоих режимах и никогда не равен подтверждённому откату.

## Сценарий 1. Повышение ресурса подшипникового узла РЦ-250

### Производственная ситуация

Сервисная статистика показывает износ подшипника выходного вала. Требуется усилить узел начиная с редуктора с заводским номером 145, сохранив старое исполнение для уже изготовленных редукторов и ремонтных комплектов. Здесь заводской номер обозначает конкретный экземпляр изделия, а не номер производственной серии. На складе есть прежние подшипники; несколько заказов ещё находятся в производстве.
...
6. Применяемость разделяет прежнее и новое исполнения по заводским номерам экземпляров: прежнее — №1–144, новое — начиная с №145. Для критических позиций отдельно определяется, достаточно ли закрепить инженерную версию или нужно точное неизменяемое состояние.
...
10. Отдел технической документации формирует предусмотренный выпускной комплект, проверяя необходимые файлы и связь с точным составом. Производство получает этот комплект. Само извещение может оставаться открытым до выполнения последующих действий по внедрению.

### Результат и ошибка

При просмотре или подготовке заказа для экземпляра №140 пользователь продолжает получать прежнее решение, а для экземпляра №145 — новое по действующим условиям. Старый опубликованный комплект не пересчитывается.

Точный снимок редуктора №145 фиксирует только свою конкретную конфигурацию и условия. Его наличие не доказывает правильность всего диапазона начиная с №145. Для диапазона отдельно проверяются применяемость, отсутствие пропусков и конфликтов, а также другие существенные условия; при необходимости другие экземпляры получают собственные точные комплекты.
...
После успеха отдельно выпускается точный комплект станка с нужными документами. Базовый снимок фиксирует не архив трёх пакетов, а полное дерево изделия от выбранного корня до листьев, включая неизменявшиеся компоненты. Пользовательский фильтр или остановка показа на третьем уровне не превращают усечённый экран в полный официальный состав. Развёртывание на каждом экземпляре остаётся отдельным процессом.
...
### Что уже было передано

Первоначальный заказ задавал расход, напор, климат и напряжение станции. Перед первой выдачей планировщик проверил состав, маршрут и обеспеченность; затем был опубликован снимок №1 и начата его передача производству. Состояние доставки и подтверждение приёма сохранялись отдельно. Это исходное обязательство нельзя стереть, просто заменив двигатель в живой карточке.
...
Выбор относится именно к этому заказу и его новой конфигурации. Он не меняет молча общую инженерную версию станции, конкретизацию всех её будущих использований или корпоративное правило подбора. Чтобы изменить такие общие решения, нужен отдельный явно заявленный предмет правки.
...
В этом примере предприятие выбрало управляемую рабочую редакцию справочника: технолог готовит изменения отдельно от используемых производством данных. Это не обязательная форма каждой таблицы Interlink: оперативный набор может состоять из изменяемых записей, а результаты испытаний — из неизменяемых фактов.

Система проверяет единицы и допустимые значения осей. Если объявлена полная сетка, все обязательные сочетания должны быть представлены без пропусков и дубликатов. Полнота сетки ещё не означает известность всех величин или пригодность расчёта: неизвестные значения обозначаются явно, а правила использования определяют, допустимы ли они для данной задачи. Для разреженной карты отсутствие ячейки означает отсутствие соответствия, пока специальное правило не определило иной смысл.
...
Различие полного набора и разреженной карты подробнее разобрано в [обзоре табличных данных](../tabular-data/README.md), [многомерных таблицах](../tabular-data/02-multidimensional-tables.md) и [координатных картах](../tabular-data/03-coordinate-maps.md).
...
## Какие исходные примеры нужны для проверки IPS

Для подтверждения переноса потребностей нужно получить реальные, обезличенные исходные и ожидаемые результаты: извещение с несколькими этапами; контекст с ручным и производным назначением; отмену незавершённой работы с зависимостями; заказ до и после замены; итерацию с файлами и восстановленным поднабором.

Результат сравнения должен описывать не только одинаковые номера объектов, но и выбранное содержание, видимость участникам, сохраняемые основания и действия при конфликте. Сценарии индексируемых данных и агентской проработки дополнительно проверяют расширения Interlink; они не используются для исключения обязательных сценариев IPS.
...
Связанные документы: [жизненный цикл](02-user-journey-and-lifecycle.md), [виды изменений](06-quick-formal-and-bundled-changes.md), [рабочие данные](03-working-state-visibility-and-concurrency.md), [допустимые замены](../substitution-and-compatibility/01-allowed-replacements.md), [матрицы совместимости](../substitution-and-compatibility/02-compatibility-matrices.md), [агенты и прослеживаемость](../agents/01-agents-and-traceability.md), [приёмка](09-status-open-questions-and-acceptance.md).
...
Основные источники: [IL-KERNEL], [IL-CHANGES], [IL-AI], [IL-PLAN-V2], [IPS-A04], [IPS-A05], [IPS-A13], [IPS-K04]. Расшифровка, статус и местонахождение — в [реестре источников](../knowledge/15-source-register.md).

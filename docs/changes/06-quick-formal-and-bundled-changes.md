...
Другой пример — перенос справочника материалов: марка стали ссылается на отсутствующий стандарт. Зависимую порцию нельзя объявить завершённой; оператор получает список требуемых оснований. Независимые порции могут завершиться отдельно, если это не нарушает связанный предметный набор. После устранения ошибки повторяется именно согласованная порция, а не вся база.
...
Подробнее: [справочные таблицы](../tabular-data/01-reference-tables.md), [полные многомерные таблицы](../tabular-data/02-multidimensional-tables.md) и [разреженные координатные карты](../tabular-data/03-coordinate-maps.md).
...
Для быстрого пути нужны реальные примеры матрицы предприятия: какая правка уже выпущенного объекта остаётся локальной, а какая требует извещения из-за влияния на заказ, расчёт или обязательный документ. Нельзя вывести этот ответ только из статуса «выпущено».

Для общего комплекта полезно разобрать механическое и технологическое извещения одного изделия. Какие результаты могут завершаться независимо, что именно считается неделимым выпуском и кто принимает междисциплинарное решение? Для импорта аналогичный вопрос — какие строки можно повторять отдельной порцией без потери полноты.
...
Связанные документы: [границы пакета](01-what-is-change-package.md), [путь пользователя](02-user-journey-and-lifecycle.md), [рабочие области](10-workspaces-and-parallel-work.md), [допустимые замены](../substitution-and-compatibility/01-allowed-replacements.md), [матрицы совместимости](../substitution-and-compatibility/02-compatibility-matrices.md), [сквозные примеры](08-end-to-end-examples.md).

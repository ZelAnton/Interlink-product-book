...
## Что уточнить при сопоставлении с IPS

Для переноса процесса недостаточно воспроизвести кнопку согласования. Нужно определить, что именно проверяет нормоконтроль на данном предприятии: отдельную правку, всю конфигурацию или комплект документов с результатами расчёта. Существенные используемые данные должны войти в точный предмет решения; смежная карточка, открытая лишь для справки, не обязана стать его участником.

Порядок применения контекста, конкретизации и применяемости подтверждается для целевой версии и настроек IPS. Приёмка должна использовать реальные исходные составы и ожидаемые результаты, включая конфликт и отсутствие подходящей версии. Совпадение названий правил не доказывает одинакового результата.
...
Связанные документы: [жизненный цикл](02-user-journey-and-lifecycle.md), [проведение и восстановление](05-commit-cancel-and-recovery.md), [снимки и доказательства](07-snapshots-evidence-and-user-history.md), [допустимые замены](../substitution-and-compatibility/01-allowed-replacements.md), [матрицы совместимости](../substitution-and-compatibility/02-compatibility-matrices.md), [агенты и прослеживаемость](../agents/01-agents-and-traceability.md).

## Основания

[IL-KERNEL], [IL-CHANGES], [IL-AI], [IL-PLAN-V2], [IPS-A04], [IPS-A05]. Актуальные нормы Interlink определяют целевое поведение; источники IPS используются для сопоставления пользовательских сценариев, а не как доказательство готовности Interlink. Расшифровка и статус источников — в [реестре источников](../knowledge/15-source-register.md).
